$(document).ready(function() {
  ...
  $('#deploy-btn-group').on('click', '.deployment', function(e) {
    ...
    $(dialog.el).addClass('deployment').addClass('deploying');
    dpd('__deployments').post({
      ...
      if (err) { 
        ...
        dialog = ui.dialog(subdomain, $('<span>Deployed to <a target="_blank" href="http://' + url + '">' + url + '</a>!</span>')).hide(5000);
      }

      $(dialog.el).addClass('deployment');
      dialog.show().closable();
    });
    e.preventDefault();
  });
  ...
  function refreshDeployments() {
    dpd('__deployments').get(function(deployments) {
      $('#deploy-btn-group .header-link').attr('href', '#');
      $('#deploy-btn-group ul .deployment').remove();
      if (deployments && deployments.length) {
        $('#deploy-btn-group .header-link')
        ...
        $('#deploy-btn-group .header-link').attr('href', '/dashboard/deployments')
