...
module.exports.loadConfig = function(basepath, server, fn) {
  ...
  var resourcesQ = loadResources(resourceTypesQ, basepath, server).then(function(resources) {
    ...
    var defaultResourcesQ = [
      ...
      loadDefaultResource(new InternalResources('__resources', { config: { resources: resources }, server: server })),
      loadDefaultResource(new Dashboard('dashboard', {server: server}))
    ];

    return q.all(defaultResourcesQ).then(function(defaultResources) {
