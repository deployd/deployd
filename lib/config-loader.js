    ...
    async.auto({
      ...
      instance: ['configJson', 'types', function(fn, results) {
        ...
        if(typeof server.options !== 'undefined'){
          o.namespace_url = server.options.resources_namespace_url || '/dpd-api/';
        }
...
function addInternalResources(server, basepath, resources, fn) {
  ...
  publicFolderPromise.then(function(publicFolder) {
    ...
    var clientLibOptions = { config: { resources: resources }, server: server };
    var dashboardOptions = { server: server };

    if( typeof server.options === 'undefined'){
      debug('server.options is undefined. Will add (show) dpd.js and dashboard in internals array');
      internals.push( new ClientLib('dpd.js', clientLibOptions) );
      internals.push( new Dashboard('dashboard', dashboardOptions) );
    }else{
      //setting new default namespacing for internal resources
      clientLibOptions.namespace_url = server.options.dpdjs_url_namespace || '/js/' ;
      dashboardOptions.dpdjs_url_namespace = server.options.dpdjs_url_namespace || '/js/';
      dashboardOptions.namespace_url = server.options.dashboard_url_namespace || '/admin/';
