...
function loadResourceDir(basepath, fn) {
  ...
  return dir.subdirs(resourceDir, function(err, subdirs) {
    if (err) return fn(err);
    fn(null, subdirs.map(function(subdir) {
      return path.relative(resourceDir, subdir).split(path.sep).join('/');;
    }).filter(function(relativePath) {
      try {
        // check if config.json exists
        fs.accessSync(path.join(resourceDir, relativePath, 'config.json'), fs.F_OK);
        return true;
      } catch (err) {
        return false;
      }
