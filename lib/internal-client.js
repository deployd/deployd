...
exports.build = function(server, session, stack) {
  ...
  baseMethods = {
    request: function(method, options, fn) {
      ...
      if (req.stack.indexOf(req.url) === -1) {
        req.stack.push(req.url);
        debug("Putting %s on stack", req.url);
        ...
        fn(null, "Recursive call to " + req.url + " detected");
      }
    }
  };
...
function createResourceClient(resource, baseMethods) {
  ...
  var r = {
    get: function(func, p, query, fn) {
      ...
      settings.path = joinPath(resource.settings.path, settings.path);
  ...
  resource.clientGenerationDo.forEach(function(func) {
    r[func] = function(path, query, fn) {
      r.do(func, path, query, fn);
    };
  });
