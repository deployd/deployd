  ...
  if(sockets) {
    sockets.on('connection', function (socket) {
      ...
      if(sid) {
        ...
        if(sessionIndex[sid])
          sessionIndex[sid].rawSocket = socket ;
...
SessionStore.prototype.createSession = function(sid, fn) {
  ...
  if(sid) {
    this.find({ id: sid }, function (err, s) {
      ...
      if (s && s.uid) {
        if (userSessionIndex[s.uid] === undefined) {
          userSessionIndex[s.uid] = [];
        }
...
SessionStore.prototype.getSession = function (uid, sid) {
  return userSessionIndex[uid][sid] || null; //TODO get Session by uid and session id
};
  ...
  this.emitToUsers = function(collection, query, event, data) {
    collection.get(query, function(users) {
      ...
      users.forEach(function(u) {
        userSessions = userSessionIndex[u.id];
  ...
  session.remove(data, function (err) {
    ...
    session.store.insert(data, function (err, res) {
      if (!err) {
        ...
        if(session.rawSocket && session.store.socketIndex)
          session.store.socketIndex[sid] = session.rawSocket ;
        if (res.uid) {
          if (userSessionIndex[res.uid] === undefined) {
            userSessionIndex[res.uid] = [];
          }
