...
function Session(data, store, sockets, rawSockets) {
  ...
  this._leaveAllRooms = function() {
    _.each(self.store.socketIndex[self.sid], function(socket) {
      _.each(_.without(socket.rooms, socket.id), function(room) {
        socket.leave(room);
      });
    });
  };
