  ...
  .use(function (req, res, next, end) {
    if(req.method === 'PUT') {
      ...
      resources.get(req.query).first(function (err, res) {
        var origPath = res && res.path;
        if(res.path === '/') res.path = '/index';
        if(err || !res) return next(err);
        if(res.type === 'Static') {
          resources.get({path: {$regex: '^' + origPath + '.*'}}, function (err, matched) {
            var remaining = matched.length;
            
            matched.forEach(function (m) {
              resources.del({path: m.path}, function () {
                // for collection urls
                if(m.path === '/') m.path = '/index';
                collection.use(m.path + '.files').del(function () {
                  collection.use(m.path + '.chunks').del(function () {
                      next();
                  });
                })
              });
