...
app.post('/users', function(req, res) {
  Users
    .notify(function(all) {
      if(all && all.length === 0) {
        // make the first user the root user
        req.body.groups = {root: 1};
      }
...
app.del('/users/:id', function(req, res) {
  user('remove', {_id: req.param('id')}, req, res);
});

// determine if app is brand new
app.isNew = function(cb) {
  Users
    .notify(function(users) {
      cb(users && users.length === 0);
    })
    .fetch()
  ;
}
