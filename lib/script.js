...
Script.loaddir = function (configPath, events, fn) {
  var remaining = 0;
  ...
  function read(file) {
    var isJS = path.extname(file) === '.js'
      , fileName = isJS && file
      , ev = fileName && fileName.substr(0, fileName.length - 3)
      , filePath = file && ev && path.join(configPath, fileName);

    if(filePath) {
      Script.load(filePath, loaded(ev));
    }
  }
  ...
  function loaded(ev) {
    ...
    return function (err, script) {
      events[ev] = script;
      // capitalize
      events[ev.substr(0, 1).toUpperCase() + ev.substr(1)] = script;
