    ...
    Object.keys(resourceConfig).forEach(function(k) {
      ...
      if(defaults[c.type]) {
        try {
          ...
          err = e;
          err.message += ' - when initializing: ' + c.type;
        }
      } else {
        err = 'cannot find type ' + c.type;
      }
    });
      ...
      case 'PUT':
        ...
        if (resource.$setAll) {
          ...
          Object.keys(ctx.body).forEach(function(k) {
            resources[id][k] = resource[k];
          });
          
          if(ctx.body.$renameFrom != ctx.body.path) {
            resources[ctx.body.path.replace('/', '')] = resource;
            delete resources[id];
            sh.mv(basepath + 'resources/' + ctx.body.$renameFrom.replace('/', ''), basepath  + 'resources/' + ctx.body.path.replace('/', ''));
          }
        }
        
        notifyType(resources[id] || resource, ctx, function (err) {
          config.saveConfig(resources, basepath, function (err) {
