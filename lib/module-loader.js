...
module.exports = function loadModules(basepath, fn) {
  ...
  var allFilesQ = q.spread([getDirQ(path.join(__dirname, 'resources'), getDirQ(path.join(basepath, 'node_modules'))],
    function(defaultResources, modules) {
      return Array.prototype.push.apply(defaultResources, modules);
    });

  var modulesQ = allFilesQ.then(function(allFiles) {
    var modules = {};

    return q.ninvoke(async, 'forEach', function(file, fn) {
      loadModule(file, function(err, module, id) {
  ...
  var moduleQ = statQ.then(function(stat) {
    if (stat.isDirectory() || path.extname(file) === '.js') {
      ...
      try {
        ...
        // TODO: test this; we don't want it to print the error twice
