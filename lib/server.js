...
Server.prototype.handleRequest = function handleRequest (req, res) {
  ...
  setupReqRes(server.options, req, res, function(err, next) {
    if(server.options.hasOwnProperty("responseHeaders")){
	for (var header in server.options.responseHeaders) {
    	     if (server.options.responseHeaders.hasOwnProperty(header)) {
        	res.setHeader(header, server.options.responseHeaders.header);
   	     }
        }
   } 
