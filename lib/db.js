...
function Db(options) {
  this.options = options;
  this._mdc = new mongodb.MongoClient(new mongodb.Server(this.options.host, this.options.port), {w:0});
}
...
Db.prototype.drop = function (fn) {
  getConnection(this, function (err, mdc, mdb) {
    mdc.open(function () {
      mdb.dropDatabase(fn);
    });
  });
...
function getConnection(db, fn) {
  if(db.connected) {
    fn(null, db._mdc, db._mdb);
  } else if(db.connecting) {
    db.once('connection attempted', function (err) {
      fn(err, db._mdc, db._mdb);
    });
  } else {
    db.connecting = true;
    db._mdc.open(function (err) {
      db._mdb = db._mdc.db(db.options.name)
      db.connecting = false;
...
function collection(store, fn) {
  ...
  getConnection(db, function (err, mdc, mdb) {
...
Store.prototype.remove = function (query, fn) {
  ...
  collection(this, function (err, col) {
    col.remove(query, {w:'journaled'}, fn);
  });
};
