...
Store.prototype.find = function (query, fn) {
  ...
  collection(this, function (err, col) {
    if(query._id) {
      if(fields) {
        col.findOne(query, fields, function (err, obj) {
          store.identify(query);
          fn(err, store.identify(obj));
        });
      } else {
        col.findOne(query, function (err, obj) {
          store.identify(query);
          fn(err, store.identify(obj));
        });
      }
      ...
      if(fields) {
        col.find(query,  fields).toArray(function (err, arr) {
...
Store.prototype.update = function (query, object, fn) {
  ...
  collection(this, function (err, col) {
    // only set properties
    col.update(query, {$set: object}, function(err) {
      store.identify(query);
      fn(err);
    });
  });
