...
Store.prototype.find = function (query, fn) {
  ...
  collection(this, function (err, col) {
    ...
    if(typeof query._id === 'string') {
      ...
      if(fields) {
        col.find(query,  fields, options).collation( { locale: 'en', strength: 2 } ).toArray(function (err, arr) {
          if (err) {
            fn(err);
            return;
          }
        ...
        col.find(query, options).collation( { locale: 'en', strength: 2 } ).toArray(function (err, arr) {
          if (err) {
            fn(err);
            return;
          }
