...
var commands = {
    ...
    listit: function () {
        ...
        console.log("\n\n\n\n\nSTUFF: ", server.resources);
    },

    resources: function () {
        if (server.resources) {
            server.resources.forEach(function (r) {
                console.log('\t' + r.name, '(' + r.config.type + ')');
                if(r.properties){
                    
                    for(var d=0; d < Object.keys(r.properties).length; d++){
                        console.log('\t\t' + r.properties[Object.keys(r.properties)[d]].name + ' | ' + r.properties[Object.keys(r.properties)[d]].type + ' | ' + r.properties[Object.keys(r.properties)[d]].order);
                    }
                        
                }
//                for(d=0; d < r.properties.length; d++){
//                    
//                   console.log('\n\t\t' + r.properties[d]); 
//                }
            });
...
function buildReplClient(dpd) {
    var client = internalClient.build(dpd, {
        isRoot: true
    });
