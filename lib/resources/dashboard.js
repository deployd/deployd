...
Dashboard.prototype.render = function(ctx, options) {
  ...
  if (layout) {
    function finish(context, render) {
      try {
        render.scripts = render.scripts || options.scripts;
        rendered = layout({context: context, render: render});  
    ...
    if (resource) {
      ...
      if (resourceType.dashboard) {
        if (resourceType.dashboardPages) {
          page = parts[1] || resourceType.dashboardPages[0];
        } else {
          page = 'index';
        }
