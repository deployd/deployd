...
Collection.prototype.handle = function (ctx) {
  ...
  switch (ctx.req.method) {
    case 'GET':
      ...
      if (typeof ctx.res.getHeader('X-Count') === 'undefined') {
        var store = this.store
          , sanitizedQuery = this.sanitizeQuery(ctx.query || {});
        delete sanitizedQuery.$skip;
        delete sanitizedQuery.$limit;

        store.count(sanitizedQuery, function (err, result) {
          if (err) return fn(err);

          console.log('ctx.res.getHeader x-count', ctx.res.getHeader('X-Count'));

          ctx.res.setHeader('Access-Control-Expose-Headers', 'X-Count');
          ctx.res.setHeader('X-Count', result);
        });
      }
