...
Collection.prototype.find = function (ctx) {
  var collection = this
    , store = this.store
    , query = ctx.query || {}
    , session = ctx.session
...
Collection.prototype.save = function (ctx) {
  var collection = this
    , store = this.store
    , session = ctx.session
    , item = ctx.body
  ...
  function put() {
    store.first({id: query.id, $fields: query.$fields}, function(err, obj) {
      ...
      if(collection.scripts.onPut) {
        collection.scripts.onPut.run(ctx, domain, function (err) {
          if(err) {
            return done(err);
          }
