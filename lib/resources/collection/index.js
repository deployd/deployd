...
Collection.prototype.configDeleted = function(resourcePath, config, fn) {
  return this.store.remove(fn);
}

Collection.prototype.configChanged = function(resourcePath, config, fn) {
  ...
  if(config.$renameFrom != config.path) {
    store.rename(ctx.body.path.replace('/', ''), function (err) {
        fn(err, ctx.body);
    });
    return;
  }
