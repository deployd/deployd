...
Collection.prototype.remove = function (ctx, fn) {
  ...
  store.find(query, function (err, result) {
    ...
    if(collection.shouldRunEvent(collection.events.Delete)) {
      var domain = {
        error: function(key, val) {
          errors = errors || {};
          errors[key] = val || true;
...
Collection.prototype.save = function (ctx, fn) {
  ...
  function put() {
    ...
    store.first({id: query.id, $fields: query.$fields}, function(err, obj) {
      ...
      function runPutEvent(err) {
        ...
        if(collection.shouldRunEvent(collection.events.Put)) {
      ...
      if (collection.shouldRunEvent(collection.events.Validate)) {
  ...
  function post() {
    ...
    if(collection.shouldRunEvent(collection.events.Post)) {
