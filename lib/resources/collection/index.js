...
Collection.prototype.save = function (ctx, fn) {
  ...
  function put() {
    ...
    store.first(sanitizedQuery, function(err, obj) {
      if(!obj) {
        if (ctx.req && ctx.req.method == 'POST' && query.id) {
          // post to create a new item with a specified id and if the obj is not exists before
          return post();
        }        
  ...
  function post() {
    ...
    if (!item.id) item.id = store.createUniqueIdentifier();
  ...
  // cannot do (ctx.req.method == 'POST') checking in here as post can also update item
  if (query.id) {
    put();
  } else {
    validatePost();
