...
Collection.prototype.load = function(fn) {
  ...
  eventNames.forEach(function(e) {
    ...
    fs.readFile(filePath, 'utf-8', function(err, val) {
      if (val) {
        events[e] = val;
        scripts[e] = new Script(val, filePath);
      }
      remaining--;
      if (remaining <= 0) {
        fn();
...
Collection.prototype.find = function (ctx, fn) {
  ...
  store.find(sanitizedQuery, function (err, result) {
    ...
    result.forEach(function (data) {
      ...
      collection.scripts.onGet.run(ctx, domain, function (err) {
        remaining--;
        if(!remaining || err) {
          done(err, result);
        }
...
/**
 * Execute the onDelete script. If it succeeds, remove all objects in a
...
Collection.prototype.remove = function (ctx, fn) {
  ...
  store.find(query, function (err, result) {
    ...
    if(collection.scripts.onDelete) {
      var domain = {
        error: function(key, val) {
          errors = errors || {};
          errors[key] = val || true;
      ...
      collection.scripts.onDelete.run(ctx, domain, done);
    } else {
      done();
    }
  });
...
Collection.prototype.save = function (ctx, fn) {
  ...
  function put() {
    store.first({id: query.id, $fields: query.$fields}, function(err, obj) {
      ...
      if(collection.scripts.onPut) {
        collection.scripts.onPut.run(ctx, domain, commit);
      } else {
        commit();
      }
    });
  ...
  function post() {
    ...
    if(collection.scripts.onPost) {
      collection.scripts.onPost.run(ctx, domain, function (err) {
        if(err) {
  ...
  if(collection.scripts.onValidate) {
    collection.scripts.onValidate.run(ctx, domain, function (err) {
