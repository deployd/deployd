...
Collection.prototype.load = function(fn) {
  this.loadScripts(Collection.events, fn);
}
...
Collection.prototype.find = function (ctx, fn) {
  ...
  store.find(sanitizedQuery, function (err, result) {
    ...
    result.forEach(function (data) {
      ...
      collection.scripts.Get.run(ctx, domain, function (err) {
        remaining--;
        if(!remaining || err) {
          done(err, result);
        }
...
Collection.prototype.remove = function (ctx, fn) {
  ...
  store.find(query, function (err, result) {
    ...
    if(collection.scripts.Delete) {
      ...
      collection.scripts.Delete.run(ctx, domain, done);
    } else {
      done();
    }
  });
...
Collection.prototype.save = function (ctx, fn) {
  ...
  function put() {
    store.first({id: query.id, $fields: query.$fields}, function(err, obj) {
      ...
      if(collection.scripts.Put) {
        collection.scripts.Put.run(ctx, domain, commit);
      } else {
        commit();
      }
    });
  ...
  function post() {
    ...
    if(collection.scripts.Post) {
      collection.scripts.Post.run(ctx, domain, function (err) {
        if(err) {
  ...
  if(collection.scripts.Validate) {
    collection.scripts.Validate.run(ctx, domain, function (err) {
