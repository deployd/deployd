...
(function() {
  ...
  vm.edit.dismiss = function() {
    setTimeout(function() {
      ...
      if (vm.selectedRow() !== vm.newRow) {
        var propAccessor = vm.edit.editProp
          , prop = vm.selectedProp()
          , row = vm.selectedRow()
          , lastValue = propAccessor();

        createUndo("Changed " + vm.selectedProp().name, function() {
          propAccessor(lastValue);
          vm.selectedProp(prop);
          vm.selectedRow(row);
        });  
      }
      vm.edit.editProp(vm.edit.editValue());
    }, 1);    
  };
  ...
  vm.edit.start = function(newVal) {
    ...
    if (typeof newVal === 'string' || typeof newVal === 'number') {
      ...
      vm.edit.editValue(JSON.stringify(vm.edit.editProp(), null, '\t'));
    } else {
