...
UserCollection.prototype.handle = function (ctx) {
  ...
  switch(ctx.req.method) {
    ...
    case 'POST':
      ...
      /* falls through */
    case 'PUT':
      if(ctx.body && ctx.body.password !== undefined) {
        if(!ctx.body.password || ctx.body.password === '') {
          return ctx.done({errors: {password: 'has to be set'}});
        }
    ...
    function loginDone(err) {
      ...
      checkAndSaveUser(function (err) {
        ...
        ctx.session.set({ path: path, uid: user.id, userhash: usernameAndPasswordHash }).save(function (err, session) {
          ...
          if (ctx.res.cookies) ctx.res.cookies.set('sid', session.id, { overwrite: true });
