        ...
        this.store.first({username: credentials.username}, function(err, user) {
          ...
          if(user) {
            ...
            if(hash === uc.hash(credentials.password, salt)) {
              ...
              /* validate session */
              var err = {};
              for(var key in uc.sessions) {
                var val = ctx.req.body[key]
                  , session = uc.sessions[key]
                  , type = session.type || 'string';

                if (validation.exists(val)) {
                  if(type === 'number') sessionData[key] = Number(val);
                  if(!validation.isType(val, type)) {
                    err[key] = 'must be a ' + type;
                  } else {
                    sessionData[key] = val;
                  }
                }
              ...
              // if (Object.keys(err).length) return ctx.done(err);
