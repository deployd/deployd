  ...
  if(method === 'Get') {
    if(Array.isArray(data)) {
      ...
      data.forEach(function(item) {
        options.this = item;
        options.context.hide = function(property) {
          if (!session.isRoot) {
...
Collection.prototype.remove = function (session, query, client, fn) {
  ...
  store.find(query, function (err) {
    if(err) return fn(err);
    collection.execListener('Delete', session, query, null, client, function (err) {
      if(err) return fn(err);
...
Collection.prototype.save = function (session, item, query, client, fn) {
  ...
  collection.execListener('Validate', session, query, item, client, function (err, item) {
    ...
    if(!query.id) {
      // generate id for use in event listener
      item.id = store.createUniqueIdentifier();
    }

    collection.execListener(query.id ? 'Put' : 'Post', session, query, item, client, function (err, item) {
      if(err) return fn(err);
