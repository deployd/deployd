...
Collection.prototype.find = function (session, query, fn) {
  ...
  store.find(query, function (err, result) {
    ...
    collection.execListener('Get', session, query, result, function (err, results) {
      if(err) return fn(err);
...
Collection.prototype.save = function (session, item, query, fn) {
  ...
  var errors = this.validate(item, !query.id);

  if(errors) return fn({errors: errors});

  collection.execListener(query.id ? 'Put' : 'Post', session, query, item, function (err, item) {
    if(err) return fn(err);

    if(query.id) {
      store.first({id: query.id}, function(err, obj) {
        if(err) return fn(err);
