...
Keys.prototype.create = function(fn) {
  ...
  this.readFile(function(err, data) {
    ...
    keys.writeFile(data, function(err) {
      if(err){
        if(err.code == 'ENOENT' && err.path == '.dpd/keys.json'){
          console.error('Path: '+err.path+' not found, creating new one.');
          // TODO: Could ask with prompt?
          fs.mkdirSync('.dpd');
          keys.writeFile(data,function(error){
            fn(error, key);
          });
        }else{
          fn(err, key);
