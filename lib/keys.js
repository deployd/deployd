...
Keys.prototype.create = function(fn) {
  ...
  if (this.path.indexOf('.dpd') === 0 && !fs.existsSync('.dpd') ){
    if( fs.existsSync('node_modules/deployd/.') ){
      fs.mkdirSync('.dpd');
    }else{
      console.error("ERROR! You don't have deployd module in node_modules directory, won't create .dpd directory");
    }
  }
  this.readFile(function(err, data) {
