...
describe('UserCollection', function() {
	describe('.handle(ctx)', function() {
		beforeEach(function() {
			...
			this.uc = new UserCollection({path: '/users', db: db.connect(TEST_DB), properties: properties});
			this.ctx = {
				req: {url: '/users', body: {}},
				res: {},
	...
	describe('.handleSession(ctx)', function() {
		it('should attach the current user to the session', function(done) {
			...
			var uc = new UserCollection({path: '/users', db: db.connect(TEST_DB), properties: properties});
